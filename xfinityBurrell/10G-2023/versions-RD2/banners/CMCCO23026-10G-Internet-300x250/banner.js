(function (factory) {
    ...
    function thereAndBack() {
      var startEase = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : Power1.easeInOut;
      var endEase = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : Power1.easeInOut;

      var ease = function ease(p) {
        if (p < 0.5) {
          return startEase(p / 0.5);
        } else {
          return endEase(1 - (p - 0.5) / 0.5);
        }
      }; // if you don't do this, at the end of the ease GSAP will set the values to the target values
      // without running them through the easing function.
    ...
    function createMainTL() {
      ...
      var frame1TL = createFrameTL(grab('#frame1')); // scaler

      frame1TL.to('#frame1 .pulse-anim', {
        duration: 1.5,
        scale: VER(1.3, {
          '160x600': 1.15,
          '728x90': 1.2,
        ...
        ease: bendTime(combine(thereAndBack(), lockedEase(0.2, 0.9)), 1.1)
      }, 0.9);
      tl.add(frame1TL, '.5'); // █▀ █▀▄ ▄▀▄ █▄ ▄█ ██▀    ▀▀▄
      // █▀ █▀▄ █▀█ █ ▀ █ █▄▄    █▄▄
...
//# sourceMappingURL=banner__Internet.js.map
